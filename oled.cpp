...
inline void OLED_Object::sendByte(uint8_t dat, bool isCMD)
{
    // cs->reset();
    csOut(0);
    dcOut(!isCMD);
...
void OLED_Object::reset(void)
{
    ...
    this->loadFont(ASCII_16_8[0], 16, 8);
}

inline void OLED_Object::setCol(uint8_t col)
{
    this->sendByte(0x10 | (col >> 4), true);
    this->sendByte(0x00 | (col & 0x0f), true);
}

inline void OLED_Object::setPage(uint8_t page)
{
